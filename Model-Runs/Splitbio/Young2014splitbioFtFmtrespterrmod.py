...
trespterr = np.array([.506, .512, .5149, .516, .520])

# For loop for different tresp values
for trespi in trespterr:
    ...
    for ft in ftx:
        ...
        kr9t = ft * kr91 * alphaCO2H2O * isoevap * rQSMOW
        kr9m = (1 - ft) * kr91 * alphaCO2H2O * isophoto * rQSMOW
        ...
        def f(y, t):
            ...
            kOXtO = (k12t * (alphart ** trespi) + k12m * (alpharm ** tresp) + k13 + k14)
...
fig1.xaxis.set_ticks(np.arange(0, 1, .2))
...
# Saving figure
plt.savefig('D17O2tvtrespterrFtmod.jpg', dpi=800)
