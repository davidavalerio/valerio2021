...
FCt = 125e15 / 12.0111 # mol C / yr from Zhang1996
...
FCm = FCt # mol C / yr from Field1998
...
fMRm = np.array([0.3, 0.4, 0.5]) # from Badger2000

# Marine biosphere production w/o MR
FO2m = FCm * pQ

# O2 production in marine biosphere taken by Mehler reaction
FO2MRm3 = FO2m * (1 / (1 - fMRm[0])) - FO2m
FO2MRm4 = FO2m * (1 / (1 - fMRm[1])) - FO2m
FO2MRm5 = FO2m * (1 / (1 - fMRm[2])) - FO2m
...
tPRRU = tPRHU + 0.004 # tPR = 0.510 offset from Angert2003 and 0.516 from Helman2005
...
# For 30, 40, and 50% marine Mehler fractions
...
# For 10, 20, and 30% marine Mehler fractions
...
fig1.set(xlim = (0.510, 0.518), ylim = (-0.6, -0.3))
...
fig1.set_title("$^{17}\Delta$ $O_2 $$_{trop}$ vs. $\Theta_{GA}$ for FCt = FCm and fMRt = 0.05")
...
# Saving figure
plt.savefig('D17O2vtGAC05.jpg', dpi=800)
