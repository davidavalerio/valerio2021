...
tGA = np.array([.512, .514, .516, .520, 0.497, 0.526])
...
for i in sol[50:60]:
    D17 = i.loc['D17_O2t'].values
    D17_ftx526.append(D17)
D17_ftx526 = np.hstack(D17_ftx526)

# Setting up figure parameters
fig1 = plt.figure(figsize = (5, 5))
fig1 = fig1.add_subplot(1, 1, 1)
fig1.set(xlim = (0, 1), ylim = (-1, -0.15))
fig1.grid()

# Plotting D17 of Wostbrock 2020
plt.hlines(-.441, 0, 1, colors='black', linestyles='dashed',
           label = "$\Delta'^{17}O$ $O_{2, Wostbrock2020}$")

# Plotting D17 of O2t as function of fraction terrestrial biosphere
fig1.plot(ftx, D17_ftx512, label='$\Theta_{PR}$ = 0.512', color='#4477aa',
          linewidth='1.5')
fig1.plot(ftx, D17_ftx514, label='$\Theta_{AOX}$ = 0.514', color='#66ccee',
          linewidth='1.5')
fig1.plot(ftx, D17_ftx516, label='$\Theta_{COX}$ = 0.516', color='#228833',
          linewidth='1.5')
fig1.plot(ftx, D17_ftx520, label='$\Theta_{COX}$ = 0.520', color='#ccbb44',
          linewidth='1.5')
fig1.plot(ftx, D17_ftx497, label='$\Theta_{marine\:MR}$ = 0.497', color='#ee6677',
          linewidth='1.5')
fig1.plot(ftx, D17_ftx526, label='$\Theta_{terrestrial\:MR}$ = 0.526', color='#aa3377',
          linewidth='1.5')
...
# Legend
fig1.legend(loc='upper left', bbox_to_anchor=(0.03, 0.50), ncol=2)

# Saving figure
plt.tight_layout()
plt.savefig('D17O2tvtGA.jpg', dpi=800)