...
tGA = 0.520 # RU global average oxygen uptake theta 
...
# Target isotopic values
isotopest = pd.DataFrame(np.array([np.nan, np.nan, np.nan, 40.4269, 94,
                                  24.046, 39.9737, np.nan, np.nan, np.nan,
                                  22.9443, 77, 12.1678, 21.3399, np.nan,
                                  np.nan, np.nan, 1.5462, 29, -0.441,
                                  0.2337]),
                         index = isotopeso)

# Target mole fraction and isotope flux values
fracfluxt = pd.DataFrame(np.array([0.21, 2.7e-4, 2.7e-4, 2e-6, 4.01e15]),
                         index = fracfluxo)
...
ratios.columns = ['Value']
isotopes['Target'] = isotopest[0]
isotopes['Percent diff'] = perdisotopes[0]
isotopes.columns = ['Per mil', 'Target', 'Percent diff']
fracflux['Target'] = fracfluxt[0]
fracflux['Percent diff'] = perdfracflux[0]
fracflux.columns = ['Mole fraction/flux', 'Target', 'Percent diff']

print(isotopes)
print(fracflux)

#Export data as excel spreadsheet
writer = pd.ExcelWriter('Y14WB2.xlsx', engine = 'xlsxwriter')
isotopes.to_excel(writer, sheet_name = 'Isotopes')
fracflux.to_excel(writer, sheet_name = 'Mole fraction + flux')
ratios.to_excel(writer, sheet_name = 'Isotope Ratios')
