...
of tropospheric O2 responds to changes in the value of the theta value of evapotranspiration.
...
for te in tevap:
    ...
    # Solve the DEs
    moles = odeint(f, y0, t, args=(te,), mxstep = 1000000)
...
# D17 as function of tevap
D17_tevap = []
for i in solutions:
    D17 = i.loc['D17_O2t'].values
    D17_tevap.append(D17)
    
D17_tevap = np.hstack(D17_tevap)

# Setting up figure parameters
fig1 = plt.figure(figsize = (5, 5))
with sns.axes_style("whitegrid"):
    fig1 = fig1.add_subplot(1, 1, 1)
fig1.set(xlim = (.505, .530), ylim = (-.4254, -.4246))
fig1.yaxis.set_major_formatter(mtick.StrMethodFormatter("{x:.4f}"))
fig1.yaxis.set_minor_locator(mtick.AutoMinorLocator(2))
fig1.xaxis.set_minor_locator(mtick.AutoMinorLocator(2))


# Plotting D17 O2t as function of theta CO2-H2O
plt.plot(tevap, D17_tevap)

# Fit linear regression to data
reg = stats.linregress(tevap, D17_tevap)
        
# D17 O2t vs. theta respiration regression slope
x1 = np.linspace(0, 100)
D17_reg = reg.slope * tevap + reg.intercept

# tevap from Young2014
tY14 = 0.520
...
# tevap theta from Hoffman 2012
tH12 = 0.522
D17H12 = reg.slope * tH12 + reg.intercept
plt.vlines(tH12, -0.65, D17H12, colors='blue', linestyles='dashed',
           label = "Hoffman 2012")
plt.hlines(D17H12, 0.505, tH12, colors='blue', linestyles='dashed')


# Legend and title
fig1.set_xlabel("$\Theta_{evap}$")
fig1.set_ylabel("$^{17}\Delta$ $O_2, $$_{trop}$ (‰)")
fig1.set_title("$^{17}\Delta$ $O_2, $$_{trop}$ vs. $\Theta_{evap}$")
fig1.legend(loc='best')

# PLotting D17 of O2t as function of tevap
fig1.plot(tevap, D17_tevap)
plt.tight_layout()
plt.savefig('D17tvtevap', dpi=800)

# Exporting D17 data as .csv
np.savetxt("D17_tevap", D17_tevap, delimiter=',')