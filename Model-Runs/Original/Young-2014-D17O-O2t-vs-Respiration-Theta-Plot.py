...
of tropospheric O2 responds to changes in the value of the average global theta value for all respiratory pathways.
...
for tr in tresp:
    ...
    # Solve the DEs
    moles = odeint(f, y0, t, args=(tr,), mxstep = 1000000)
...
# D17 as function of tresp
D17_tresp = []
for i in solutions:
    D17 = i.loc['D17_O2t'].values
    D17_tresp.append(D17)
    
D17_tresp = np.hstack(D17_tresp)

# Setting up figure parameters
fig1 = plt.figure(figsize = (5, 5))
with sns.axes_style("whitegrid"):
    fig1 = fig1.add_subplot(1, 1, 1)
fig1.set(xlim = (.505, .530), ylim = (-.65, -.075))
...
plt.plot(tresp, D17_tresp)

# Fit linear regression to data
reg = stats.linregress(tresp, D17_tresp)
        
# D17 O2t vs. theta respiration regression slope
x1 = np.linspace(0, 100)
D17_reg = reg.slope * tresp + reg.intercept

# Average respiration theta from Young 2014
tY14 = .5149
D17Y14 = reg.slope * tY14 + reg.intercept

plt.vlines(tY14, -0.65, D17Y14, colors='black', linestyles='dashed',
           label = "Young 2014 avg.")
plt.hlines(D17Y14, 0.505, tY14, colors='black', linestyles='dashed')

# PR Angert 2003 D17 O2t
tA03 = 0.506
D17A03 = reg.slope * tA03 + reg.intercept

plt.vlines(tA03, -0.65, D17A03, colors='blue', linestyles='dashed',
           label = "Angert 2003 PR")
plt.hlines(D17A03, 0.505, tA03, colors='blue', linestyles='dashed')

# PR Helman 2005 D17 O2t
tH05 = 0.512
D17H05 = reg.slope * tH05 + reg.intercept

plt.vlines(tH05, -0.65, D17H05, colors='red', linestyles='dashed',
           label = "Helman 2005 PR")
plt.hlines(D17H05, 0.505, tH05, colors='red', linestyles='dashed')

# COX Angert 2003
tCOX = 0.516
D17COX = reg.slope * tCOX + reg.intercept

plt.vlines(tCOX, -0.65, D17COX, colors='green', linestyles='dashed',
           label = "Angert 2003 COX")
plt.hlines(D17COX, 0.505, tCOX, colors='green', linestyles='dashed')

# Respiration Ash 2019
tAsh = 0.520
D17Ash = reg.slope * tAsh + reg.intercept

plt.vlines(tAsh, -0.65, D17Ash, colors='purple', linestyles='dashed',
           label = "Ash 2019 resp")
plt.hlines(D17Ash, 0.505, tAsh, colors='purple', linestyles='dashed')

# Legend and title
fig1.set_xlabel("$\Theta_{resp}$")
fig1.set_ylabel("$^{17}\Delta$ $O_2, $$_{trop}$ (‰)")
fig1.set_title("$^{17}\Delta$ $O_2, $$_{trop}$ vs. $\Theta_{resp}$")
fig1.legend()

# PLotting D17 of O2t as function of tresp
fig1.plot(tresp, D17_tresp)
plt.tight_layout()
plt.savefig('D17tvtresp', dpi=800)

# Exporting D17 data as .csv
np.savetxt("D17_tresp", D17_tresp, delimiter=',')