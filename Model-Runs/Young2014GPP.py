...
# Solve the ODEs for varying GPP
GPP = np.linspace(0.5, 1.50)
k12GPP = GPP * k12
k21GPP = GPP * k21

solutions = []

for k12, k21 in zip(k12GPP, k21GPP):
    # Solve the DEs
    moles = odeint(f, y0, t, args=(zip(k12, k21),), mxstep = 1000000)
...
fig1.plot(GPP, D17_GPP)

# Legend and title
fig1.set_xlabel("Relative GPP from modern")
fig1.set_ylabel("$^{17}\Delta$ $O_2$$_{trop}$ (‰)")
fig1.set_title("$^{17}\Delta$ $O_2$$_{trop}$ vs. Relative GPP")
fig1.legend(loc='best')

# PLotting D17 of O2t as function of GPP
fig1.plot(GPP, D17_GPP)
plt.tight_layout()
plt.savefig('D17tvGPP.jpg', dpi=800)

# Exporting D17 data as .csv
np.savetxt("D17_GPP", D17_GPP, delimiter=',')