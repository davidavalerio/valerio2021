...
tresp = 0.516 # global average oxygen uptake theta consistent with Wostbrock2020
...
fracfluxo = np.array(['xO2', 'xCO2t', 'xCO2s', 'xO3s', 'xO1Ds', 'xOs', 'xJ'])
...
fracflux = pd.DataFrame(np.full((1, 7), 0, dtype=float), columns = fracfluxo)
...
fracflux = pd.DataFrame(np.array([xO2, xCO2t, xCO2s, xO3s, xO1Ds, xOs,
                                  xJ]),
                        index =fracfluxo)
...
isotopest = pd.DataFrame(np.array([np.nan, np.nan, np.nan, 40.4269, 94,
                                  23.2, 39.9737, np.nan, np.nan, np.nan,
                                  22.9443, 77, np.nan, 21.3399, np.nan,
                                  np.nan, np.nan, 1.5462, 29, np.nan,
                                  0.2337]),
                         index = isotopeso)

# Target mole fraction and isotope flux values from Young 2014
fracfluxt = pd.DataFrame(np.array([0.21, 2.7e-4, 2.7e-4, 2e-6, 2.58e-19,
                                   2.58e-13, 4.01e15]), index = fracfluxo)
...
writer = pd.ExcelWriter('Young2014sbmodWBSS.xlsx', engine = 'xlsxwriter')
