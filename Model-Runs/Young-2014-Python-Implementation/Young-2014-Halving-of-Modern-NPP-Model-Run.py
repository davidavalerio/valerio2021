...
# Initial conditions for pre-NPP drop
...
# Solve the DEs for pre-NPP drop steady state
molespre = odeint(f, y0pre, tpre, mxstep = 10000000)

# Time pre and post-NPP drop to plot
tpre = 3.5e4
tpost = 3.5e4

# Steady state pre NPP drop
molespret = molespre[-int(tpre * 10 + 1): -1]

# Final molar amounts of species from pre-NPP drop steady state
molespre = molespre[-1]

# Initial conditions for evaluating affect of NPP drop
y0NPPdrop = molespre
k21 = .5 * k21

# Time grid for evaluating affect of NPP drop increase
tNPPdrop = np.arange(0, tpost, 0.1)

# Solve the DEs for effect of anthropogenic CO2 increase
molesNPPdrop = odeint(f, y0NPPdrop, tNPPdrop, mxstep = 100000000)

# Set up output dataframe
molespret = pd.DataFrame(molespret, columns = moleso, index = np.arange(0, tpre, 0.1))
molesNPPdrop = pd.DataFrame(molesNPPdrop, columns = moleso, index = np.arange(0, tpost, 0.1))
molesNPPdrop = molespret.append(molesNPPdrop)
...
R18_Os = atomZ(isok2[0], molesNPPdrop['Qs'], molesNPPdrop['Os'])
R17_Os = atomZ(isok2[0], molesNPPdrop['Xs'], molesNPPdrop['Os'])
R18_O1Ds = atomZ(isok2[1], molesNPPdrop['Q1Ds'], molesNPPdrop['O1Ds'])
R17_O1Ds = atomZ(isok2[1], molesNPPdrop['X1Ds'], molesNPPdrop['O1Ds'])
R18_O2s = atomZ(isok2[2], molesNPPdrop['OQs'], molesNPPdrop['O2s'])
R17_O2s = atomZ(isok2[2], molesNPPdrop['OXs'], molesNPPdrop['O2s'])
R18_CO2s = atomZ(isok2[3], molesNPPdrop['COQs'], molesNPPdrop['CO2s'])
R17_CO2s = atomZ(isok2[3], molesNPPdrop['COXs'], molesNPPdrop['CO2s'])
R18_O3s = atomZ(isok2[4], molesNPPdrop['OOQs'], molesNPPdrop['O3s'])
R17_O3s = atomZ(isok2[4], molesNPPdrop['OOXs'], molesNPPdrop['O3s'])
R18_O2t = atomZ(isok2[5], molesNPPdrop['OQt'], molesNPPdrop['O2t'])
R17_O2t = atomZ(isok2[5], molesNPPdrop['OXt'], molesNPPdrop['O2t'])
R18_CO2t = atomZ(isok2[6], molesNPPdrop['COQt'], molesNPPdrop['CO2t'])
R17_CO2t = atomZ(isok2[6], molesNPPdrop['COXt'], molesNPPdrop['CO2t'])
R18_O2b = atomZ(isok2[7], molesNPPdrop['OQb'], molesNPPdrop['O2b'])
R17_O2b = atomZ(isok2[7], molesNPPdrop['OXb'], molesNPPdrop['O2b'])
R18_O2g = atomZ(isok2[8], molesNPPdrop['OQg'], molesNPPdrop['O2g'])
R17_O2g = atomZ(isok2[8], molesNPPdrop['OXg'], molesNPPdrop['O2g'])
...
xO2 = xO2(molesNPPdrop['O2t'], molesNPPdrop['OXt'], molesNPPdrop['OQt'], airt)

# Calculate mole fraction of CO2 in the troposphere
xCO2t = xCO2(molesNPPdrop['CO2t'], molesNPPdrop['COXt'], molesNPPdrop['COQt'], airt)

# Calculate mole fraction of CO2 in the stratosphere
xCO2s = xCO2(molesNPPdrop['CO2s'], molesNPPdrop['COXs'], molesNPPdrop['COQs'], airs)

# Calculate mole fraction of O3 in the stratosphere
xO3s = xO3(molesNPPdrop['O3s'], molesNPPdrop['OOXs'], molesNPPdrop['OOQs'], airs)
...
isotopesNPPdrop = pd.DataFrame(np.full(((10 * int(tpost)), 27), 0, dtype=float), columns = isotopeso)
fracfluxNPPdrop = pd.DataFrame(np.full(((10 * int(tpost)), 5), 0, dtype=float), columns = fracfluxo)

# Assign calculated isotope values to dataframe
isotopesNPPdrop = pd.DataFrame(np.array([d18_Os, d18_O1Ds, d18_O2s, d18_CO2s,
...
fracfluxNPPdrop = pd.DataFrame(np.array([xO2, xCO2t, xCO2s, xO3s, xJ]),
                        index =fracfluxo)

#%% Plots

# Time grid to plot
tplot = np.append(np.arange(-(tpre), 0, 0.1), np.arange(0, tpost, 0.1))

# Plot D17 O2t vs. time
fig1 = plt.figure(figsize = (5, 5))
with sns.axes_style("whitegrid"):
    fig1 = fig1.add_subplot(1, 1, 1)
fig1.set(xlim = (-(tpre), tpost), ylim = (-.8, -.1))
plt.plot(tplot, D17_O2t, color='black')
plt.xlabel("Time (years)")
plt.ylabel("$^{17}\Delta$ O$_{2, trop}$")
plt.title("$^{17}\Delta$ O$_{2, trop}$ Halving NPP")
plt.tight_layout()
plt.savefig("D17O2tHalfNPP.jpg", dpi = 800)

# Plot XCO2 O2t vs. time
fig2 = plt.figure(figsize = (5, 5))
with sns.axes_style("whitegrid"):
    fig2 = fig2.add_subplot(1, 1, 1)
fig2.set(xlim = (-(tpre), tpost), ylim = (0, 0.0012))
plt.plot(tplot, xCO2t, color='blue')
plt.xlabel("Time (years)")
plt.ylabel("X(CO$_2$)$_{trop}$")
plt.title("X(CO$_2$)$_{trop}$ Halving NPP")
plt.tight_layout()
plt.savefig("XCO2HalfNPP.jpg", dpi = 800)

# Plot XO2 vs. time
fig3 = plt.figure(figsize = (5, 5))
with sns.axes_style("whitegrid"):
    fig3 = fig3.add_subplot(1, 1, 1)
fig3.set(xlim = (-(tpre), tpost), ylim = (0, 0.4))
plt.plot(tplot, xO2, color='green')
plt.xlabel("Time (years)")
plt.ylabel("X(O$_2$)")
plt.title("X(O$_2$) Halving NPP")
plt.tight_layout()
plt.savefig("XO2HalfNPP.jpg", dpi = 800)    

# Plot d18O O2t vs. time
fig4 = plt.figure(figsize = (5, 5))
with sns.axes_style("whitegrid"):
    fig4 = fig4.add_subplot(1, 1, 1)
fig4.set(xlim = (-(tpre), tpost), ylim = (10, 35))
plt.plot(tplot, d18_O2t, color='red')
plt.xlabel("Time (years)")
plt.ylabel("$\delta$$^{18}$ O$_{2, trop}$")
plt.title("$\delta$$^{18}$ O$_{2, trop}$ Halving NPP")
plt.tight_layout()
plt.savefig("d18O2tHalfNPP.jpg", dpi = 800)

# 
#Export data as excel spreadsheet
#writer = pd.ExcelWriter('Young2014NPPdrop.xlsx', engine = 'xlsxwriter')
#molesNPPdrop.to_excel(writer, sheet_name = 'Moles')
#isotopesNPPdrop.to_excel(writer, sheet_name = 'Isotopes')
#fracfluxNPPdrop.to_excel(writer, sheet_name = 'Mole fraction flux')
#writer.save()