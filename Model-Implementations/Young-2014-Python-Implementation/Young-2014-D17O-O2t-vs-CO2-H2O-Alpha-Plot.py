...
for tCH in tCO2H2O:
    # Solve the DEs
    moles = odeint(f, y0, t, args=(tCH,), mxstep = 1000000)
...
# D17 as function of tCO2H2O
D17_tCO2H2O = []
for i in solutions:
    D17 = i.loc['D17_O2t'].values
    D17_tCO2H2O.append(D17)
    
D17_tCO2H2O = np.hstack(D17_tCO2H2O)

# Setting up figure parameters
fig1 = plt.figure(figsize = (5, 5))
with sns.axes_style("whitegrid"):
    fig1 = fig1.add_subplot(1, 1, 1)
fig1.set(xlim = (.505, .530), ylim = (-.429, -.424))
...
plt.plot(tCO2H2O, D17_tCO2H2O)

# Fit linear regression to data
reg = stats.linregress(tCO2H2O, D17_tCO2H2O)
        
# D17 O2t vs. theta respiration regression slope
x1 = np.linspace(0, 100)
D17_reg = reg.slope * tCO2H2O + reg.intercept

# Nominal equilibration theta
tN = 0.528
D17N = reg.slope * tN + reg.intercept
plt.vlines(tN, -0.65, D17N, colors='black', linestyles='dashed',
           label = "Nominal equilibration")
plt.hlines(D17N, 0.505, tN, colors='black', linestyles='dashed')

# Average CO2-H2O theta from Cao and Liu 2014
tCL11 = 0.525
D17CL11 = reg.slope * tCL11 + reg.intercept
plt.vlines(tCL11, -0.65, D17CL11, colors='blue', linestyles='dashed',
           label = "Cao and Liu 2011")
plt.hlines(D17CL11, 0.505, tCL11, colors='blue', linestyles='dashed')


# Legend and title
fig1.set_xlabel("$\Theta_{CO2-H2O}$")
fig1.set_ylabel("$^{17}\Delta$ $O_2$$_{trop}$ (‰)")
fig1.set_title("$^{17}\Delta$ $O_2$$_{trop}$ vs. $\Theta_{CO2-H2O}$")
fig1.legend(loc='best')

# PLotting D17 of O2t as function of tCO2H2O
fig1.plot(tCO2H2O, D17_tCO2H2O)
plt.tight_layout()
plt.savefig('D17tvtCO2H2O', dpi=800)

# Exporting D17 data as .csv
np.savetxt("D17_tCO2H2O", D17_tCO2H2O, delimiter=',')